...
class VisionInference:
    ...
    def _plant_likelihood(self, data: bytes):
        ...
        try:
            ...
            # If the heuristic itself fails, do not pretend that the image was
            # verified as a plant. Let the upload continue only as unverified.
    ...
    def analyze(self, image_bytes: bytes, expected_crop: str|None=None):
        self._ensure_loaded()
        plant_score, green_ratio, yellow_ratio, center_green, skin_ratio = self._plant_likelihood(image_bytes)
        ...
        if (plant_score < 0.055 and green_ratio < 0.025 and yellow_ratio < 0.018 and skin_ratio > 0.30) or (plant_score < 0.035 and green_ratio < 0.012 and yellow_ratio < 0.010):
