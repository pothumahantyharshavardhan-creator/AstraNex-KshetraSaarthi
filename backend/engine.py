...
from .services.alert_engine import build_alerts, overall_level
from .services.crop_profiles import stage_supported, get_profile
...
def analyze_field(x: AnalysisInput, vision: dict | None = None)->dict:
    ...
    irrigation=irrigation_decide(m,t,h,x.weather,x.recent_rainfall_mm,x.recent_irrigation_hours,x.growth_stage,water,excess,crop=x.crop)
    stage_note=None
    if get_profile(x.crop) is not None and not stage_supported(x.crop, x.growth_stage):
        stage_note='Stage-specific guidance unavailable for this crop/growth-stage combination.'
    ...
    # The score, the status label and the alert colour must never contradict each other. The weighted
    # sum alone let a severe (100 %) disease reading score 78 -> "GOOD" on a red-flagged field.
    if color=='red': health_score=min(health_score,49)
    elif color=='amber': health_score=min(health_score,74)
    ...
    data_quality=compute_data_quality(
        {'soil_moisture':m,'temperature':t,'humidity':h,'weather':x.weather,'recent_rainfall_mm':x.recent_rainfall_mm},
        sensor_reliability=x.sensor_reliability, image_quality=x.image_quality,
    )
    result={
      ...
      'stage_guidance':stage_note,
    }
    result['alerts']=build_alerts(result, data_quality=data_quality)
    result['alert_level']=overall_level(result['alerts'])
    return result
