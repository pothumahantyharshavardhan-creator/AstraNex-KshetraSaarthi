...
class AnalyzeRequest(BaseModel):
    ...
    location_name: Optional[str] = Field(None, max_length=240)

    @field_validator('crop','growth_stage','weather','connectivity','condition')
    @classmethod
    def safe_text(cls, v):
        v=str(v).strip().lower()
        if not v: raise ValueError('field cannot be empty')
    ...
    @field_validator('soil_moisture','temperature','humidity','recent_rainfall_mm','recent_irrigation_hours','sensor_reliability','image_quality')
    @classmethod
...
class SensorRequest(BaseModel):
    ...
    @field_validator('soil_moisture','temperature','humidity','timestamp','battery')
    @classmethod
    def finite_sensor_values(cls, v):
